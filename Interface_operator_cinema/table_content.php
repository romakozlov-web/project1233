<<?php
if (!isset($current_db) || !defined('DEFAULT_DB')) {
    $current_db = defined('DEFAULT_DB') ? DEFAULT_DB : 'project_Kozlov';
}
if (!isset($current_table)) {
    $current_table = $_GET['table'] ?? '';
}
if (empty($current_table)) {
    echo '<div class="alert alert-warning">Table not specified. Please select a table from the sidebar.</div>';
    return;
}
